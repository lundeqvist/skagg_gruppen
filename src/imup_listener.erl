...
%% @doc To be called by the main server process to be able to listen to port Port and HandlerPID will receive the messages, RPID will receive the tuple {server_started, Socket} if successful.
...
%% ReceiverPID = Process which can receive the tuple {server_started, Socket}
%% 1> ServerSocket = imup_listen(ReceiverPID, 5555, HandlerPID)
%% {server_started,#Port<0.669>}'''
%% </div>      
%% @end
%%-------------------------------------------------------------------------
...
acceptor(LSocket, ParentPID) ->
    ...
    %%{ok, Socket} = gen_tcp:accept(LSocket),
    Socket = case gen_tcp:accept(LSocket) of
		 {ok, Sock} ->
		     Sock;
		 {error, closed} ->
...
process_data(Socket, Data, HandlerPID) when is_binary(Data) ->
	...
	{send_to_users, ReceiverList, Msg} ->
	    TmpFun = (fun(RID) ->
			     HandlerPID ! {get_client_pid, self(), RID},
			     receive
				 {pid, SockPID} ->
				     convey_message(SockPID, {message, Msg}),
				     {ok, delivered};
	    ...
	    lists:foreach(TmpFun, ReceiverList),
	    {ok, delivered};
	{host_game, GameID} ->
